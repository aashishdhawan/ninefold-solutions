...
```sh
$ EXPORT=1 UNOPTIMIZED=1 bun run build
```
...
>
> ```sh
> $ EXPORT=1 UNOPTIMIZED=1 BASE_PATH=/myblog bun run build
> ```
>
> => In your code, `${process.env.BASE_PATH || ''}/robots.txt` will print `"/myblog/robots.txt"` in the `out` build (or only `/robots.txt` if `bun run dev`, ie: on localhost:3000)
